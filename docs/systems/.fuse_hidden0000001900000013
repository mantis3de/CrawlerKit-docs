...
Every wall in CrawlerKit is registered as an `EdgeBlocker` in the grid data. For a wall to be physically passable the `EdgeBlockerMarker` on the wall GameObject must have **Is Passable** checked. This is separate from whether the wall is *visible* — visibility is controlled by the animator and mesh activation.
...
- `Idle_Hidden → Revealing` — trigger `Reveal`, no Exit Time, Transition Duration 0.
- `Revealing → Idle_Revealed` — Exit Time on, Transition Duration 0.
- `Idle_Revealed → Restoring` — trigger `Restore`, no Exit Time, Transition Duration 0. *(Only if Permanent = false)*
- `Restoring → Idle_Hidden` — Exit Time on, Transition Duration 0. *(Only if Permanent = false)*
