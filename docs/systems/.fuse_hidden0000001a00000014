...
2. **Add Component → Party Bobbing.**
3. Optionally drag your camera `Transform` into **Camera** — the component auto-detects it from children if left empty.
4. Tune **Bob Height** and **Steps Per Second** to taste.
...
- If `PartyVisuals.IsAnimating` is `true` — the bob phase advances and the camera is offset by `sin(phase) * bobHeight` on the Y axis.
- If the party is **not** animating — the phase resets to `0` and the camera lerps back to its rest position (`initialLocalOffset`) at 10× speed, so it snaps back quickly without a jarring cut.
...
- **Both** — the `PartyVisuals` bob shapes the step, and `PartyBobbing` adds an overlaid sway.
- **`PartyVisuals` only** — set `PartyBobbing` disabled or remove it; you still get the per-step bob.
- **`PartyBobbing` only** — set `PartyVisuals.bobHeight` to `0`; the continuous sway replaces the step bob.

---

## Troubleshooting

??? failure "Component disables itself on play"
...
??? failure "Camera bobs but never returns to rest"
...
??? failure "Bob is too strong / too weak"
