...
4. Assign footstep `AudioClip`s and optionally a hit sound — the component creates its own `AudioSource`.
...
| Field | Default | What it does |
|---|---|---|
| **Footstep Sounds** | — | One or more `AudioClip`s. A random one plays on each `OnPartyMoved` event. |
...
- `OnPartyMoved` → `AnimateMove`
- `OnPartyTurned` → `AnimateTurn`
- `OnPartyBlocked` → `AnimateBounce`
...
??? failure "Party teleports instead of sliding"
...
??? failure "Footstep sounds don't play"
...
??? failure "The party snaps back to origin on load"
