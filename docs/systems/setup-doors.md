...
- [Button Door](setup-door-button.md) — `WallButton` + `TriggerOpenDoorTimed` or `TriggerOpenDoor`
- [Pressure Plate Door](setup-pressure-plate.md) — `PressurePlate` + `TriggerOpenDoor`
- [Keyhole Door](setup-keyhole.md) — `WallKeyhole` + `TriggerOpenDoor` via `DoorKeyholeSetup`

For multi-source setups (logic gates, sequence puzzles, levers) see [Door Types — 10 Examples](door-types.md).
