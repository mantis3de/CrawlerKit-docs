...
### Added
- **Dedicated save folder** — `SaveSystem` now writes all `save_*.json` files into a `CrawlerKit/` sub-folder of `Application.persistentDataPath` instead of the bare persistent data directory. The folder name is a single constant, `SaveSystem.SaveFolderName` (set to `""` to disable the sub-folder), and every read / write / delete path resolves through the shared `SaveSystem.SaveDirectory`, so the location stays consistent across the asset. See [Save System](systems/save-system.md).
