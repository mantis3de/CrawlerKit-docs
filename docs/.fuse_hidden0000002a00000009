...
Common questions from Asset Store customers. Can't find your answer? Reach out through your store account or the [GitHub repository](https://github.com/Mantis3de/CrawlerKit).
...
??? question "Do I need to know how to code?"
    No. The five visual editors — Character Generator, Dungeon Generator, Inventory Editor, Spell Editor and Enemy Spawner — let you build complete content without scripting. Coding is only needed if you want to extend the systems, and the [API Reference](api.md) covers that.

??? question "How do I make my first dungeon?"
...
??? question "How is dungeon data stored? Is generation done at runtime?"
    Dungeons are generated and edited entirely in the editor, then baked to a single `GridData_<SceneName>.json` file in a `Resources` folder. At runtime the level is simply loaded from that JSON, so there is **no procedural generation cost** in the shipped game. See [Architecture](architecture.md).

??? question "How do the editors talk to the runtime?"
...
??? question "Can I use only part of the framework — say, just the Dungeon Generator?"
...
??? question "Is there a save/load system?"
    Yes. The Save System persists party, enemy and grid state. Any component can participate by implementing `ISaveable` and registering with `ISaveSystem` — see the [API Reference](api.md).

??? question "How does loot work?"
    You author **loot tables** in the [Inventory Editor](editors/inventory-editor.md) with weighted entries and three roll modes (Pick One, Pick N, Roll All), then assign them to enemies via drag-and-drop in the [Enemy Spawner](editors/enemy-spawner.md). A built-in simulator lets you test drops without entering Play mode.

??? question "Can enemies respawn or patrol?"
    Yes. The `EnemySpawner` component supports a respawn delay, an initial spawn delay, and patrol routes with **Loop** or **PingPong** modes. See the [Enemy Spawner](editors/enemy-spawner.md) page.

## Troubleshooting

??? question "The CrawlerKit menu doesn't appear."
    The framework probably hasn't compiled. Check the Console for errors, confirm your Unity version meets the [requirements](getting-started/installation.md), and let Unity finish importing. Reimporting the package usually resolves it.

??? question "The Dungeon Generator warns there's no GridCoreBridge / Cell Size."
